...
export const OrderDetails = () => {
  const { loading, name, order } = useSelector(
    (store: RootReducerType) => store.orderDetails,
  );
  return (
    <div className={styles.wrapper}>
      <h1 className="text text_type_digits-large pb-15">{order.number}</h1>
      <p className="text text_type_main-medium pb-4">{name}</p>
      <img
        src={!loading ? doneImage : waitImage}
        alt={"done.svg"}
        className={styles.icon}
      />
      <p className="text pt-15 pb-4">
        {!loading ? "Ваш заказ начали готовить" : "Ожидание ответа сервера..."}
      </p>
